...
class GameGUI:
    ...
    def intialise_dynamic(self):
        ...
        for x in range (6):
            for y in range (7):
                ...
                self.button = tkinter.Button(self.mw, image = self.default_image, text = " ", font = ('Arial 30 bold'), command = lambda i = x, j = y: self.clicked_btn(i,j), height= 120 , width = 140, bg ="light blue") #the command (lambda) retrieves the x and y values and runs the clicked_btn method for every button
    ...
    def loading(self):  #reading the text file to create the board saved in the text file
                        #this method does not work (index error)
        self._board = self.gboard.get_gameboard()   
        
        file = open ("Connect4_Saves.txt", "r") #open file
        for r_1 in range (6):
            line = file.readline()  #read file as a list
    ...
    def clicked_btn (self,x, y):
        ...
        if button["text"] == " ":
            ...
            if p.get_name() == "Computer Player":   #if current player is computer
                time.sleep(2)  #pause move for 2 seconds to create 'thinking'
                print ("ME") 
